...
export class StravistiX {
		...
		Helper.getFromStorage(this.extensionId, StorageManager.storageLocalType, StravistiX.versionInstalledKey, (response: any) => {
			...
			if (!response.data || !response.data.version) {
				...
				if (response.data.version && response.data.version !== this.appResources.extVersion) {
					...
					StorageManager.setCookieSeconds("stravissimo_athlete_update_done", false, 0); // Remove stravistix_athlete_update_done cookie to trigger athlete commit earlier
	...
	protected handleTrackTodayIncomingConnection(): void {
		...
		const userHasConnectSince24Hour: boolean = (StorageManager.getCookie("stravissimo_daily_connection_done") == "true");

		if (env.debugMode) {
			console.log("Cookie 'stravissimo_daily_connection_done' value found is: " + userHasConnectSince24Hour);
		}
		...
		if (!userHasConnectSince24Hour) {
			...
			if (env.debugMode) {
				console.log("Cookie 'stravissimo_daily_connection_done' not found, send track <IncomingConnection> / <" + accountType + "> / <" + eventName + ">");
			}
			...
			if (env.debugMode) {
				console.log("Cookie 'stravissimo_daily_connection_done' exist, DO NOT TRACK IncomingConnection");
			}
		}
	}
	...
	protected handleActivitiesSyncFromOutside() {
		...
		const allowSync = (urlParams.stravissimoSync === "true") ? true : false;
		if (!allowSync) {
			return;
		}
