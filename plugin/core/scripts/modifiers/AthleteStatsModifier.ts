...
export class AthleteStatsModifier implements IModifier {
    ...
    protected init(activities: any): void {
        ...
        this.progressThisYear.find("#stravissimo_yearProgress_incVirtualRides").prop("checked", StorageManager.getCookie("stravistix_yearProgress_incVirtualRides") === "true");
        this.progressThisYear.find("#stravissimo_yearProgress_incVirtualRides").on("click", () => {
            StorageManager.setCookie("stravissimo_yearProgress_incVirtualRides", $("#stravistix_yearProgress_incVirtualRides").prop("checked"), 365);
            this.handleProgressStatsForceRefresh();
        });

        this.progressThisYear.find("#stravissimo_yearProgress_incCommutes").prop("checked", StorageManager.getCookie("stravistix_yearProgress_incCommutes") === "true");
        this.progressThisYear.find("#stravissimo_yearProgress_incCommutes").on("click", () => {
            StorageManager.setCookie("stravissimo_yearProgress_incCommutes", $("#stravistix_yearProgress_incCommutes").prop("checked"), 365);
            this.handleProgressStatsForceRefresh();
        });
    }
    ...
    protected formatData(activities: any[]): any[] {
        ...
        const includeVirtualRide: boolean = (StorageManager.getCookie("stravissimo_yearProgress_incVirtualRides") === "true");

        const includeCommutes: boolean = (StorageManager.getCookie("stravissimo_yearProgress_incCommutes") === "true");
    ...
    protected renderTrendArrow(value: number, formatFunction: Function): string {
        ...
        const formatted = formatFunction ? formatFunction(value) : value;
        return "<span title='" + (value < 0 ? "-" : (value > 0 ? "+" : "")) + formatted + "' style='cursor: help; color:" + (value < 0 ? "red" : (value > 0 ? "green" : "black")) + "'>" + (value < 0 ? "\u25BC" : (value > 0 ? "\u25B2" : "=")) + "</span>";
    }

    protected processData(activities: any[], progressThisYear: any, distanceInKilometers: any, elevationInMeters: any, distanceUnit: any): void {
        ...
        for (let i: number = 0; i < types.length; i++) {
            ...
            const $table: JQuery = $("<table class='athletesStatTable' id='athletesStatActivityType" + i + "' style='display: none;'><thead><tr><th>Year</th><th>Distance</th><th>Elevation</th><th>Time</th></tr></thead><tbody></tbody></table>");
            ...
            for (j = 0, max = yearsList.length; j < max; j++) {
                ...
                const item: any = yearsList[j];
                item.distance /= 1000;
                if (j < (yearsList.length - 1)) {
                    const previousYearItem: any = yearsList[j + 1];
    ...
    public modify(): void {
        ...
        const prepareChart = (activities: any) => {
            let i: any,
                j: any,
                ...
                processData = () => {
                    ...
                    for (i = 0, max = activities.length; i < max; i++) {
                        ...
                        const yearlyData: any = data[activity.y];
                        const activityDate: Date = new Date(activity.y, activity.m, activity.d);
                        const activityTime: number = activityDate.getTime();
                        ...
                        if (yearTarget > 0) {
                            ...
                            if (currentTargetType) {
                                ...
                                switch (currentDataType) {
                                    ...
                                    case 5:
                                        const avg30DayDistance: number = data[currentYear + 1].target / numberOfDays * 30;
            ...
            const svg = d3.select("#athleteStatChart").append("svg")
                        ...
                        if (day < item.values.length && !item.target) {
                            ...
                            if (item.target) {
                                switch (currentDataType) {
                                    case 1:
                                    case 3:
                                        const showValue: number = (item.values[1] - item.values[0]) / numberOfDays * day;
