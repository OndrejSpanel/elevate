...
export class WindyTyModifier implements IModifier {
	...
	protected modifyPage(): void {
		...
		html += "<div class='title' style='cursor: pointer;' id='stravissimo_weather_title'>Weather</div>";
		...
		$("#pagenav").append($(html)).each(() => {
			...
			$("#stravissimo_weather_title").click((evt: JQuery.Event) => {

				evt.preventDefault();
				evt.stopPropagation();

				if ($("#stravissimo_weather_title").is(":visible")) {
					$("#stravissimo_weather_title").slideUp();
				} else {
					$("#stravissimo_weather_title").slideDown();
				}
